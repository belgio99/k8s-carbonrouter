...
WAIT_MIN = float(os.getenv("BENCHMARK_WAIT_MIN", "0.01"))
WAIT_MAX = float(os.getenv("BENCHMARK_WAIT_MAX", "0.05"))
...
class RampingLoadShape(LoadTestShape):
    ...
    Load pattern:
    - 0-120s: Ramp from 200 to 1000 users (linear increase, spike during high carbon)
    - 120-300s: Hold at 1000 users (sustained pressure during transition)
    - 300-600s: Drop to 400 users (steady processing during low carbon)
    """
    ...
    def tick(self):
        ...
        if run_time < 120:
            # Phase 1: Ramp up from 200 to 1000 over 2 minutes
            # Linear interpolation: 200 + (1000-200) * (t/120)
            user_count = int(200 + (800 * run_time / 120))
            spawn_rate = 100  # Aggressive spawn rate
            return (user_count, spawn_rate)

        elif run_time < 300:
            # Phase 2: Hold at 1000 users for 3 minutes (120-300s)
            return (1000, 100)

        elif run_time < 600:
            # Phase 3: Drop to 400 users for remaining 5 minutes (300-600s)
            return (400, 100)
